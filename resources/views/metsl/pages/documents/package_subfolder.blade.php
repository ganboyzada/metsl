...
<h1 class=" text-lg ps-5 font-medium text-gray-700 dark:text-gray-200">
    Documents in  {{ $package_subfolder->name??'' }} / {{ $package_subfolder->subFolders[0]->name??'' }}
    </h1>
	...
	async function get_documents(page = 1){
		if(localStorage.getItem("project_tool") == 'documents'  || localStorage.getItem("project_tool") == 'activities'){
			if(localStorage.getItem("project_tool") == 'documents'){
                ...
                var package = `{{ $package_subfolder->id }}`;
                var subfolder_id = `{{ $package_subfolder->subFolders[0]->id??0 }}`;
				let url = 	`/project/documents/all?package_id=${package}&subfolder_id=${subfolder_id}&page=${page}&DocNO=${DocNo}&orderBy=${orderBy}&orderDirection=${orderDirection}`;
